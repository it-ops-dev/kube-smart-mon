...
exec 1>/tmp/metrics-tmp

[ -z "$MY_HOSTNAME" ] && export MY_HOSTNAME=`hostname`
[ -z "$WORK_DIR" ] && export WORK_DIR="/opt/"
...
parse_smartctl_attributes() {
  local disk="$1"
  local disk_type="$2"
  local labels="hostname=\"${MY_HOSTNAME}\",disk=\"${disk}\",type=\"${disk_type}\""
...
parse_smartctl_info() {
  local smart_available=0 smart_enabled=0 smart_healthy=0
  ...
  echo "device_info{hostname=\"${MY_HOSTNAME}\",disk=\"${disk}\",type=\"${disk_type}\",vendor=\"${vendor}\",product=\"${product}\",revision=\"${revision}\",lun_id=\"${lun_id}\",model_family=\"${model_family}\",device_model=\"${device_model}\",serial_number=\"${serial_number}\",firmware_version=\"${fw_version}\"} 1"
  echo "device_smart_available{hostname=\"${MY_HOSTNAME}\",disk=\"${disk}\",type=\"${disk_type}\"} ${smart_available}"
  echo "device_smart_enabled{hostname=\"${MY_HOSTNAME}\",disk=\"${disk}\",type=\"${disk_type}\"} ${smart_enabled}"
  echo "device_smart_healthy{hostname=\"${MY_HOSTNAME}\",disk=\"${disk}\",type=\"${disk_type}\"} ${smart_healthy}"
}
...
echo "smartctl_version{hostname=\"${MY_HOSTNAME}\",version=\"${smartctl_version}\"} 1" | format_output
...
for device in ${device_list}; do
  ...
  active=1
  echo "smartctl_run{hostname=\"${MY_HOSTNAME}\",disk=\"${disk}\",type=\"${type}\"}" "$(TZ=UTC date '+%s')"
  # Check if the device is in a low-power mode
  /usr/sbin/smartctl -n standby -d "${type}" "${disk}" > /dev/null || active=0
  echo "device_active{hostname=\"${MY_HOSTNAME}\",disk=\"${disk}\",type=\"${type}\"}" "${active}"
...
mv /tmp/metrics-tmp $WORK_DIR/metrics